 ...
 * 
 * @author Diane Mueller modified from Allison Obourn and Marty Stepp
 * @date   8/7/2017
 * @version CSC370 Fall 2017
 */
...
public class MelodyGUI implements ActionListener, StdAudio.AudioEventListener {
	...
	JButton playButton; // buttons in the GUI
	...
	public void onAudioEvent(StdAudio.AudioEvent event) {
		// update current time
		if (event.getType() == StdAudio.AudioEvent.Type.PLAY || event.getType() == StdAudio.AudioEvent.Type.STOP) {
	...
	private void createComponents() {
		...
		String currentDir = System.getProperty("user.dir") + File.separator + RESOURCE_FOLDER + File.separator
				+ MELODY_FOLDER;
		...
		appendButton = GuiUtils.createButton("Append", icon + "append.gif", 'A', this);
		...
		currentTimeSlider.setPreferredSize(new Dimension(300, currentTimeSlider.getPreferredSize().height));
	...
	private void doLayout() {
		...
		ImageIcon logoIcon = new ImageIcon(
				RESOURCE_FOLDER + File.separator + ICONS_FOLDER + File.separator + "notes.gif");
		overallLayout.add(GuiUtils.createPanel(new JLabel(logoIcon)));
		overallLayout.add(GuiUtils.createPanel(statusLabel));
		overallLayout.add(GuiUtils.createPanel(titleLabel));
		overallLayout.add(GuiUtils.createPanel(artistLabel));
		...
		overallLayout.add(GuiUtils.createPanel(reverseButton, appendButton));
		overallLayout.add(GuiUtils.createPanel(new JLabel("Tempo: "), changeTempoSpinner, changeTempoButton));
	...
	/**
	 * @return - the estimated current time within the overall song, in seconds.
	 */
	 ...
	 */
	private void loadFile(String cmd) throws IOException {
		...
		if (cmd == "Append") {
			Melody appendedMelody = new Melody(title, artist, song);
			System.out.println("Appending");
			melody.append(appendedMelody);
			System.out.println("Appending complete.");
		} else {
			melody = new Melody(title, artist, song);
			System.out.println("Loading complete.");
		}
		System.out.println("Melody: " + melody);
		titleLabel.setText(melody.getTitle());
		artistLabel.setText(melody.getArtist());
		changeTempoSpinner.setValue(1.0);
		setCurrentTime(0.0);
		updateTotalTime();
		doEnabling();
	...
	/**
	 * Reads the notes in the file storing them in an array. The notes will
	 * appear in the same order in the Queue as they did in the file.
	 * 
	 * @param input
	 *            - facilites file input of notes
	 * @return - returns a Queue filled with the notes specified in the passed
	 *         in Scanner
	 */
	private QueueADT<Note> read(Scanner input) {
		QueueADT<Note> song = new ArrayQueue<Note>();

		while (input.hasNext()) {
			double duration = input.nextDouble();
			Pitch pitch = Pitch.valueOf(input.next());
			if (pitch.equals(Pitch.R)) {
				song.add(new Note(duration, input.nextBoolean()));
			} else {
				song.add(new Note(duration, pitch, input.nextInt(), Accidental.valueOf(input.next()),
						input.nextBoolean()));
			}
		}
		return song;
		...
		if (melody != null) {
			...
			Thread playThread = new Thread(new Runnable() {
				public void run() {
					...
					System.out.println("Playing \"" + title + "\", by " + artist + " (" + duration + " sec)");
					melody.play();
	 ...
	 * 
	 * @param -
	 *            the time to be set for this melody
	 */
