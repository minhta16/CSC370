...
public class HangmanMain  {
    ...
    public static void playGame(Scanner console, HangmanGame hangman) {
        while (hangman.numGuessesLeft() > 0 && hangman.pattern().contains("-")) {
            System.out.println("wrong guesses left: " + hangman.numGuessesLeft());
            if (DEBUG) {
                System.out.println("debugging help words left: " + hangman.wordsLeft());          	
            }
            System.out.println("guessed letters   : " + hangman.guessedLetters());
            System.out.println("current : " + hangman.pattern());
            System.out.print("Your guess? ");
            char ch = console.next().toLowerCase().charAt(0);
            if (hangman.guessedLetters().contains(ch)) {
                System.out.println("You already guessed that");
            } else {
                int count = hangman.recordGuess(ch);
                if (count == 0) {
    ...
    public static void showResults(HangmanGame hangman) {
        ...
        String answer = hangman.wordsLeft().iterator().next();
